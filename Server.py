...
class Server(object):
    ...
    def search_migration_plan(self,**kwargs):
        ...
        """
        具体选举过程如下：
            通过选择过载控制器下的交换机，依次判定迁入到目标控制器后，所形成的负载方差大小。
            首先对交换机负载进行排序，从大到小，依次加入迁移集合，直到负载均衡度最小即控制器之间的负载差异最小
        """
        #排序
        # sw_load={order:{sw,pktin_speed}}
        #load_list=[pktin_speed,....]
        sorted_sw_load = {k: v for k, v in sorted(
            switches_pkt_load.items(), key=lambda item: item[1]['pktin_speed'], reverse=True)}

        sw_load = {i + 1: (k, v['pktin_speed']) for i, (k, v) in enumerate(sorted_sw_load.items())}

        load_list = [sw_load[i+1] for i, k in enumerate(sorted_sw_load.keys())]

        for order,load in enumerate(load_list):
    ...
    def monitor(self):  # 2s打印拓扑
        while 1:
            ...
            for k,v in self.switches_pktin_load.items():
                self.log.info(f'=====c{k}=====')
                for s,t in v.items():
                    self.log.info(f'sw:{s} speed:{t.get("pktin_speed")}')
