...
class Controller(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.status = False
        self.controller_id = None #控制器ID-1代表子网号
        ...
        self.access_table = {}  # {(sw,port) : {area_id:xx,ip:host_ip} 边缘交换机的端口IP 发生交换机迁移需要更改此项
    ...
    def register_access_info(self, dpid, in_port, ip=None,area_id=None):
        ...
        if in_port in self.access_ports[dpid]:
            if key in self.access_table:
                ...
                print(f'注册arp：{key},ip:{ip},area_id:{area_id}')
                self.access_table[key]["ip"] = ip
                self.access_table[key]["area_id"] = area_id
    ...
    def arp_register(self,Arp_pkt,dpid,in_port):
        ...
        if (dpid, in_port) not in self.access_table:
            # 如果该arp未注册或arp过期，当接收到ARP请求，会将IP的子网ID划分到所属area,只适用于网络初始化，后续的注册就直接由Server负责全局维护
        ...
        # 判断源地址的子网是否为当前area,当发生过交换机迁移，需要再次判断
    ...
    def _packet_in_handler(self,ev):
        ...
        datapath=msg.datapath
        dpid=datapath.id
        if dpid not in self.dps.keys():
            self.dps[dpid]=datapath
            self.log.warning(f'交换机：{dpid}连接到此控制器')
